...
%
% NOTE: The systems also include (currently) Pulseq unsupported
%       configurations:
%       - resonanceFrequencies - An array of the resonance frequencies.
%         (See also resonanceBandwidths.)
%       - resonanceBandwidths - A range per resonance frequency to assume
%         as resonant. If, for example, the resonance frequency is 1000 Hz
%         and the bandwidth is 100 Hz, then the resonance range is from 950
%         Hz (1000 - 100/2 Hz) to 1050 Hz (1000 + 100/2 Hz).
...
% Initialize with pulseq defaults (including temporary gradient limits).
%
% Pulseq sometimes adds new parameters to the default system parameters.
% This ensures we get all the necessary fields. We will later override the
% gradient limits for different gradient "modes".
SystemNoGrad = mr.opts ;

% Fill system parameters (which are not gradient limits)
...
% Pulseq unsupported configurations.
% Definitions of forbidden frequencies (Based on VE12U AP04 which added an
% extra forbidden frequency range).
SystemNoGrad.resonanceFrequencies = [1100, 550, 367] ; % [Hz]
SystemNoGrad.resonanceBandwidths = [300, 100, 55] ; % [Hz]
